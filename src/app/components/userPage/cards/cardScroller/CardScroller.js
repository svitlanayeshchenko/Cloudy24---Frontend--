    ...
    render() {
        return (
            <div className="card-scroller">
                <div><input className="prev b-prev" type="button" value="&lt;" /></div>
                <UserCard cardInfo={this.card} />
                <div><input className="next b-next" type="button" value="&gt;" /></div>
                <div className="card-info">
                    <div className="card-balance">{roundBalance(this.card[0].balance)} {this.card[0].currency}</div>
                    <div className="credit-item-name">Карта {this.card[0].name}</div>
